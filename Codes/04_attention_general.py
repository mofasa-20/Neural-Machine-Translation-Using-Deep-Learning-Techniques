...
if __name__ == "__main__":
    ...
    model  = encoder_decoder(encoder_inputs_length = threshold, decoder_inputs_length = threshold, 
                            score_fun = "general",  att_units = attention_units,
    ...
    weight_path = os.path.join(create_data_folder('weights'), 'Attention_General.h5')
    print(f"weight to save path : {weight_path}")
    model.save_weights(weight_path)

    ## creating loss plot folder
    plots_path = create_data_folder('plots') 
    loss_plot(history.history["loss"], os.path.join(plots_path, 'attention_general_loss.jpeg'))
    ...
    print('BLEU score for Attention General Model is {:.4f}.'.format(score/len(xval_english)))
    print('Attention General modelling is completed.')
